...
const Autodeebee = require('hyperbeedeebee/autodeebee')
    ...
    if(collection === 'metadb' && _data.cores && _data.__version === this._dbVersion) {  
      ...
      if(_data.blacklisted) {
        ...
        const peerInfo = {
          ...
          blind: peer.blind,
          cores : {
            writer: _data.cores.writer,
            meta: _data.cores.meta
          }
        }

        await this.metadb.update({peerPubKey: peer.peerPubKey}, peerInfo, { upsert: true })
