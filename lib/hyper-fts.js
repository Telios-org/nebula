...
class HyperFTS {
  ...
  async deIndex({ db, name, query }) {
    ...
    const { value: props } = await bee.get('idxProps')
    ...
    for (let token of tokens) {
      await batch.del(`idx:${token}:${doc._id.toHexString()}`)
    }
  ...
  async search({ db, name, query, opts }) {
    ...
    return Promise.all(docIdsSorted.slice(0, limit).map(async docId => {
      const doc = await db.findOne({ _id: ObjectID(docId)})
      return doc
    }))
  }
